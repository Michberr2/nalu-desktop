...
export default function AIPanel() {
  ...
  const send = async () => {
    ...
    try {
      await streamChat(history, {
        ...
        signal: ctrl.signal,
        onRoute: (name) => {
          ws.setRouteName(name) // shift the background photo to match the active model, like the website
  ...
  return (
    <div className="flex w-[360px] shrink-0 flex-col overflow-hidden rounded-2xl border border-glass/[0.1] bg-panel/75 backdrop-blur-2xl">
      {/* header + mode switch */}
